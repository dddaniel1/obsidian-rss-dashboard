...
- Fixed full-text loading requesting article images with root-relative paths (for example `/assets/images/<article>/figure.svg`) against Obsidian's app origin and logging `net::ERR_FILE_NOT_FOUND` for `app://obsidian.md/...` URLs: fetched article documents now get a `<base>` element pointing at the article URL before Readability extraction, already-mangled `app://obsidian.md` image/link URLs are repaired against the article site, relative `srcset` candidates on `img` and `source` elements are resolved, and reader hero images only accept safe http(s) URLs.

- Fixed feed preview cards requesting article images with feed-relative paths (for example `assets/images/<article>/figure.svg`) against Obsidian's app origin and logging `net::ERR_FILE_NOT_FOUND`: preview images extracted from feed content, media, or enclosure URLs are now resolved against the feed URL and dropped if they cannot become a safe http(s) URL.

- Fixed hotlink-protected preview and article images by retrying failed loads through Obsidian with source-aware Referer headers (article, article origin, and image host origin) and a User-Agent fallback for image hosts that reject Electron requests, while preserving lazy-load image URLs and releasing temporary image resources when views are redrawn or closed.
- Fixed dashboard library source selection resetting to Local on restart: switching between Local and FreshRSS in the dashboard header dropdown or configuring the default library in Sync settings now persists across Obsidian restarts and view openings, automatically opening the preferred library when FreshRSS sync is connected, with graceful fallback to Local when disconnected.
- Optimized dashboard performance and memory allocations: replaced quadratic `concat` array cloning across feed loops in `DashboardView` (`getFilteredArticles`, `getScopeArticlesForEmptyState`, `getTotalArticlesCountForCurrentView`, and `getAllArticles`) with direct mutations and `flatMap`; optimized FreshRSS sync state snapshotting in `SyncService.snapshot()` by replacing deep `structuredClone` with shallow copy; eliminated $O(N \times M)$ quadratic array filtering in `DashboardSyncSource.refresh()` by grouping articles by `feedId` in a single pass; throttled saved-article file existence verifications in `DashboardView` to once every 60 seconds with early exit when no saved articles exist; and added asynchronous image decoding (`decoding: "async"`) to Feed view hero cover images.
- Fixed article list view flickering during podcast and video playback from FreshRSS feeds by debouncing playback progress persistence, updating local state directly without invoking remote sync runtime actions or notifying dashboard subscribers, and preventing continuous background server synchronizations.
...
- Fixed FreshRSS folders missing from the Add feed folder selector when the server omits the optional folder type field from its Google Reader API response, lists categories only in the subscription list, or the default Uncategorized value filters out existing folders.
